...
int slurmctld_extrae_trace_fini(List job_list, struct node_record *node_table, int node_record_count)
{
	...
	for(i = 0; i < node_record_count - 1; i++) {
		fprintf(trace_fp, "%d,", node_table[i]->node_ptr->cpus);
	}
	fprintf(trace_fp, "%d):%d", node_table[i]->node_ptr->cpus, list_count(node_list));
	/* print app list */
...
void _start_thread(int cpu_id, int app_id, int task_id, int th_id)
{
	timeval fini_time; 
	gettimeofday(&fini_time, NULL);
...
int _stop_thread(int cpu_id)
{
	timeval fini_time;
	gettimeofday(&fini_time, NULL);
	long elapsed = (fini_time.tv_sec-init_time.tv_sec) * 1000000 + fini_time.tv_usec - init_time.tv_usec;
	
	sprintf(extrae_threads[cpu_id].entry, "%s:%d:%d", extrae_threads[cpu_id].entry, elapsed, RUNNING);
